...
from lab_utils_common_ import np, plt, dlblue, dlorange, sigmoid, dldarkred, gradient_descent
...
#   @output.capture()  # debug
    ...
    def resize_sq(self, bcid):
        """ Resize the checkbox """
        # Access the 'rectangles' directly via the attributes of CheckButtons
        # But we will approach this differently, as rectangles aren't directly accessible.

        # Use the 'rectangles' list of 'CheckButtons' to resize
        rect = bcid.rectangles[0]  # Access the first rectangle (checkbox visual part)
        
        # Get the current height and multiply to resize
        h = rect.get_height()
        rect.set_height(3 * h)  # Resizing the height by a factor of 3

        # Update the bounding box's y-values
        ymax = rect.get_bbox().y1
        ymin = rect.get_bbox().y0
        
        # Update the lines that represent the checkboxes' position
        bcid.lines[0][0].set_ydata([ymax, ymin])
        bcid.lines[0][1].set_ydata([ymin, ymax])

        # Trigger the canvas to redraw to reflect changes
        self.fig.canvas.draw()
